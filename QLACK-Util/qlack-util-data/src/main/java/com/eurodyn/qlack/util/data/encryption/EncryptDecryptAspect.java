...
/**
 * An aspect handling encryption/decryption annotations. To properly use this
 * aspect you need to: 1/ Annotate your model/DTO fields with {@link Encrypted}.
 * 2/ In your service methods, annotate your "getter" methods with {@link
 * Decrypt} and your "setter/save" methods with {@link Encrypt}.
 *
 * This aspect works with method signatures directly exposing a class annotated
 * as in (2) above as well as Spring's {@link org.springframework.data.domain.Pageable}
 * wrapper.
 *
 * The actual encryption/decryption algorithm to be used is left to the user of
 * this aspect by providing a bean implementing the {@link EncryptorDecryptor}
 * interface.
 */
@Aspect
@Component
public class EncryptDecryptAspect {
  ...
  /**
   * Processes an object by either encrypting or decrypting the content of its
   * annotated fields.
   *
  ...
  private Object process(Object o, MODE mode)
    ...
    final List<Field> fieldsListWithAnnotation = FieldUtils
      .getFieldsListWithAnnotation(o.getClass(), Encrypted.class);
  ...
  public Object encrypt(ProceedingJoinPoint pjp, Encrypt encrypt)
    throws Throwable {
  ...
  public Object decrypt(ProceedingJoinPoint pjp, Decrypt decrypt)
    throws Throwable {
      ...
      ((Page) results).map(o -> {
        try {
          ...
          LOGGER.log(Level.SEVERE,
            MessageFormat.format("Could not decrypt data: {0}.", o), e);
          return o;
        }
      });
    } else {
