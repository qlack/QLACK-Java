...
public class MethodLogger {
  ...
  /**
   * Enable logging actual parameter values. If disabled the parameters type
   * is shown
   */
  ...
  private Object logMethodOfAnnotatedClass(ProceedingJoinPoint point)
    throws Throwable {
    return logged(point);
  }

  @Around("loggedAnnotatedMethod() && @annotation(Logged)")
  private Object logAnnotatedMethod(ProceedingJoinPoint point)
    throws Throwable {
    return logged(point);
  }
  ...
  /**
   * Logs method execution, parameters and performance (in msec) in the
   * following format mypackage.MyClass##method([params]): in XX[msec]
   *
  ...
  /**
   * Checks precedence between flags declared in application.properties
   * (global) and flags declared as parameters of the {@link
   * com.eurodyn.qlack.util.logger.Logged} annotation.
   *
   * @param globalProp property flag declared in application.properties file
   * @param annotationProp property flag declared as parameter of the {@link
   * com.eurodyn.qlack.util.logger.Logged} annotation.
   * @return true/false
   */
