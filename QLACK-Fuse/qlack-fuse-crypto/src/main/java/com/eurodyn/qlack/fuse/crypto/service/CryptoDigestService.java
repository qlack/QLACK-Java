...
public class CryptoDigestService {
  ...
  public String hmacSha256(final String secret, final String message)
    ...
    final SecretKeySpec secretKey = new SecretKeySpec(secret.getBytes(),
      HMACSHA256);
   ...
   * Generates a random seed.
   *
   * @param length the number of bytes of the seed. Note that this is *not*
   * the length of the return value as the return value is Base64 encoded
   * @return a Base64 encoded version of the generated seed
   * @throws NoSuchAlgorithmException thrown when no algorithm is found for
   * encryption
   */
  public String generateSecureRandom(int length)
    ...
    return Base64.encodeBase64String(
      SecureRandom.getInstanceStrong().generateSeed(length));
  }
