...
/**
 * A provider of information regarding the security algorithms and services
...
public class CryptoInfoService {
  ...
  public List<SecurityProviderDTO> getSecurityProviders() {
    return Arrays.stream(Security.getProviders())
      .flatMap(
        o -> Arrays.asList(
          new SecurityProviderDTO(o.getName(), o.getVersion(), o.getInfo()))
          .stream()
      ).collect(Collectors.toList());
  }

  /**
   * Returns the security services provided by a specific provider available
   * in the runtime.
   *
   * @param providerName the provider to inquiry for available services
   * @return a list containing the found services
   */
  public List<SecurityServiceDTO> getSecurityServices(
    final String providerName) {
    return getSecurityProviders().stream()
      .filter(o -> o.getName().equals(providerName))
      .flatMap(
        o -> Arrays.asList(Security.getProvider(o.getName()).getServices())
          .stream())
      .flatMap(o -> o.stream().flatMap(
        x -> Arrays.asList(
          new SecurityServiceDTO(providerName, x.getAlgorithm(), x.getType()))
          .stream()))
      .collect(Collectors.toList());
  }
   ...
   * @return a list containing the found services
   */
  public List<SecurityServiceDTO> getSecurityServicesForAlgorithmType(
  ...
  public String prettyPrint(final List<SecurityServiceDTO> services) {
    ...
    services.stream().sorted(
      ...
      .forEach(service -> s.append(
        "Provider: " + service.getProvider() + ", Algorithm: " + service
          .getAlgorithm()
          + ", Type: "
          + service
          .getType() + "\n"));
  ...
  /**
   * Checks if unlimited strength cryptography is available. The check is
   * taking place by comparing the maximum size available of an AES key. If
   * this method returns false, you can enable unlimited strength cryptography
   * by downloading https://www.oracle.com/technetwork/java/javase/downloads/jce8-download-2133166.html
   * (or in JDK &gt; 8 by setting Security.setProperty("crypto.policy",
   * "unlimited")).
   *
