...
/**
 * A client to communicate with ES. This client is using the {@link RestClient}
 * implementation of the ES Java client.
 *
...
public class ESClient {
  ...
  private static final Logger LOGGER = Logger
    .getLogger(ESClient.class.getName());
  ...
  /**
   * Post-construct client initialization method. It initializes a REST High
   * level client using properties provided in the <b>application.properties</b>
   * file.
   */
  @PostConstruct
  public void init() {
    LOGGER.log(Level.WARNING, "Initialising connection to ES: {0}",
      properties.getEsHosts());
    // WARN Initialising connection to ES: http://localhost:9200 -
    // c.e.qlack.fuse.search.util.ESClient
    /** Process Http hosts for ES */

    final HttpHost[] httpHosts = Arrays
      .stream(properties.getEsHosts().split(","))
      .map(host -> new HttpHost(host.split(":")[1],
        Integer.parseInt(host.split(":")[2]),
        host.split(":")[0]))
