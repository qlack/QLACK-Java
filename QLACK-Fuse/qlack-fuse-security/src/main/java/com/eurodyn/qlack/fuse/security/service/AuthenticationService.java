...
public class AuthenticationService {
   ...
   * @param applicationSessionId the sessionId of the application the User is
   * using
   ...
   * Provides access to Spring's authentication method.
   *
   * @param user An object containing the username, the password and the
   * sessionId of the User to be authenticated
   * @return generated jwt
   * @throws AuthenticationException If authentication fails
   */
  ...
  private String generateJWT(Authentication authentication,
    String applicationSessionId) {
    UserDetailsDTO authenticatedUser = (UserDetailsDTO) authentication
      .getPrincipal();

    String userId = authenticatedUser.getId();
    String sessionId = userService.login(userId, applicationSessionId, true)
      .getSessionId();
    ...
    if (jwtIncludeRoles) {
      ...
      return JWTUtil.generateToken(
        new JWTGenerateRequestDTO(jwtSecret, authenticatedUser.getUsername(),
          claims,
          jwtExpiration));
    } else {
      return JWTUtil.generateToken(
