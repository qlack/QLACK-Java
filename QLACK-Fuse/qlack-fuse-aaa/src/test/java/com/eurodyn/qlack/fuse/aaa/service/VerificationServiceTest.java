...
public class VerificationServiceTest {
    ...
    public void testCreateVerificationTokenWithData() {
        when(userRepository.findById(userID)).thenReturn(Optional.of(user));
        verificationService.createVerificationToken(userID, verificationToken.getExpiresOn(), verificationToken.getData());
        verify(verificationTokenRepository, times(1)).save(any());
    }

    @Test
    public void testVerifyToken() {
        when(verificationTokenRepository.findById(verificationToken.getId())).thenReturn(Optional.of(verificationToken));
    ...
    public void testGetTokenPayloadNull() {
        when(verificationTokenRepository.findById(verificationToken.getId())).thenReturn(Optional.empty());
        String foundPayload = verificationService.getTokenPayload(verificationToken.getId());
