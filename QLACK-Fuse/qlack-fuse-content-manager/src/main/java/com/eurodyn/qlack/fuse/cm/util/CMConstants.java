/*
 * Copyright 2014 EUROPEAN DYNAMICS SA <info@eurodyn.com>
 *
 * Licensed under the EUPL, Version 1.1 only (the "License").
 * You may not use this work except in compliance with the Licence.
 * You may obtain a copy of the Licence at:
 * https://joinup.ec.europa.eu/software/page/eupl/licence-eupl
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the Licence is distributed on an "AS IS" basis,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the Licence for the specific language governing permissions and
 * limitations under the Licence.
 */
package com.eurodyn.qlack.fuse.cm.util;

public class CMConstants {

  public static final String ATTR_NAME = "NAME";
  public static final String ATTR_CREATED_BY = "CREATED BY";
  public static final String ATTR_LAST_MODIFIED_ON = "LAST MODIFIED ON";
  public static final String ATTR_LAST_MODIFIED_BY = "LAST MODIFIED BY";
  public static final String ATTR_LOCKED_ON = "LOCKED ON";
  public static final String ATTR_LOCKED_BY = "LOCKED BY";
  public static final String CREATED_ON = "CREATED ON";
  public static final String LOCKABLE = "LOCKABLE";
  public static final String VERSIONABLE = "VERSIONABLE";
}
