...
public class ConcurrencyControlService {
  ...
  /**
   * Locks a node, so that it cannot be modified. If the node is already locked then an exception is
   * thrown.
   *
   * @param nodeID The id of the node to be locked.
   * @param lockToken A token used to lock the node. All future operations altering this node will
   * be blocked while the node is locked unless the token used to lock the node is passed to the
   * operation.
   * @param lockChildren Whether child nodes of this node should also be locked
   * @param userID The ID of the user who locked this node. This will be used simply as metadata for
  ...
  /**
   * Unlocks a node. Before this method attempts to unlock a node it checks to see whether it has
   * been previously locked or not therefore you can safely use it even if you do not know the
   * current lock state of a node.
   *
   * @param nodeID The id of the node to be unlocked.
   * @param lockToken The token with which the node was locked which will be used to unlock the
   * node
   * @param overrideLock If false then this method will not attempt to use the passed lock token but
   * will just unlock the node by setting null as the current token.
   * @param userID The ID of the user who locked this node. This will be used simply as metadata for
   * the lock and will not be taken into account when checking if a locked node can be edited.
  ...
  /**
   * Checks whether an ancestor folder has lock conflict and returns the first one it will find. A
  ...
  public FolderDTO getAncestorFolderWithLockConflict(String nodeID, String lockToken) {
    ...
    return node.getParent() != null ? getAncestorFolderWithLockConflict(node.getParent().getId(),
        lockToken) : null;
  }

  /**
   * Checks whether a descendant node has lock conflict  and returns the first one it will find.  A
   * lock conflict in this case happens when the provided lock token, is different from the
